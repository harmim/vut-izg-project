 ...
 *
 * The main purpose of vertex puller is construction of vertices that are sent
 * to vertex shader.
...
/**
 * @brief This struct represents configuration for one reading head of vertex
 * puller.
 *
 * A vertex is composed of N vertex attributes (typically position, normal,
 * texture coord, ...).
 * A vertex attribute is composed of N components (1,2,3,4).
 * Type of each component is the same.
 * Each vertex has different data in its attributes (position of each vertex is
 * different).
 * The data for vertex attributes are stored in buffers.
 */
struct GPUVertexPullerHead
{
	///< offset in bytes where this attribute starts for first vertex
	size_t offset;
	///< distance in bytes between attributes for two consecutive
	size_t stride;
	int enabled; ///< is this attribute enabled?
	const void *buffer; ///< buffer that contains this attribute
};
...
{
	///< reading heads for each vertex attribute
	GPUVertexPullerHead heads[MAX_ATTRIBUTES];
	///< indices to vertices, if it is NULL -> indexing is not used
 ...
 *
 * An attribute could be set to zero pointer - this attribute must not be used
 * in vertex shader.
 * If an attribute is not set to zero pointer, it points to location of data for
 * vertex attribute.
 * It is programmers duty to reinerpret data to suitable format.
 */
struct GPUVertexPullerOutput
{
	const void *attributes[MAX_ATTRIBUTES];  ///< list of attributes
};
 ...
 * Alternative function in OpenGL is
 * <a href="https://www.opengl.org/sdk/docs/man4/html/glCreateVertexArrays.xhtml">
 ...
 */
void cpu_createVertexPullers(GPU gpu, size_t n, VertexPullerID *arrays);

/**
 * @brief This function sets vertex attribute in vertex puller.
 *
 * Its alternative functions in OpenGL is
 * <a href="https://www.opengl.org/sdk/docs/man4/html/glBindVertexBuffer.xhtml">
 * glVertexArrayVertexBuffer
 * </a>.
 *
 * This function sets vertex attribute in vertex puller
 * \see GPUVertexPullerHead GPUVertexPullerConfiguration
 *
 * @param gpu GPU handler
...
void cpu_setVertexPullerHead(
	GPU gpu, VertexPullerID puller, size_t headIndex, BufferID buffer,
 ...
 * <a href="https://www.khronos.org/registry/OpenGL-Refpages/gl4/html/glVertexArrayElementBuffer.xhtml">
...
void cpu_setIndexing(
	GPU gpu, VertexPullerID puller, BufferID buffer, size_t indexSize
);
 ...
 *
 * Its alternative function in OpenGL is
 * <a href="https://www.khronos.org/registry/OpenGL-Refpages/gl4/html/glEnableVertexAttribArray.xhtml">
 ...
 */
void cpu_enableVertexPullerHead(GPU gpu, VertexPullerID puller, size_t head);
 ...
 */
void cpu_disableVertexPullerHead(GPU gpu, VertexPullerID puller, size_t head);
 ...
 */
void cpu_bindVertexPuller(GPU gpu, VertexPullerID id);
