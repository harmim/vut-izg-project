...
{
	const char *groundTruthFile = "../tests/output.bmp";
	int32_t windowWidth = 500;
	int32_t windowHeight = 500;
	const char *applicationName = "izgProjekt2017";
	...
	if (argc > 1 && strcmp(argv[1], "-c") == 0)
	{
		if (argc < 3)
		{
			fprintf(
				stderr,
				"ERROR: expected path to reference image after -c command line "
				"argument. Image is located: tests/output.bmp\n"
			);
		}
		else
		{
	...
	// pointers to methods
	const ON_INIT onInits[] = {
		phong_onInit, triangleExample_onInit,
	};

	const ON_EXIT onExits[] = {
		phong_onExit, triangleExample_onExit,
	};

	const ON_DRAW onDraws[] = {
		phong_onDraw, triangleExample_onDraw,
	};

	const size_t nofMethods = sizeof(onInits) / sizeof(onInits[0]);
