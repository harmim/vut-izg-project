 ...
 * attribute and type of each vertex attribute.
 */
struct GPUPrimitive
{
	...
	///< interpolation types of vertex attributes
...
struct GPUTriangle
{
	...
	Vec3 coords[WEIGHTS_PER_BARYCENTRICS];
};
...
VertexIndex gpu_computeGLVertexID(
	const VertexIndex *indices, VertexShaderInvocation vertexShaderInvocation
);
...
const void *gpu_computeVertexAttributeDataPointer(
	const GPUVertexPullerHead *head, VertexIndex gl_VertexID
);
...
void gpu_runVertexPuller(
	...
	const GPUVertexPullerConfiguration *pullerConfiguration,
...
void gpu_runPrimitiveAssembly(
	GPU gpu, GPUPrimitive *primitive, size_t nofPrimitiveVertices,
	const GPUVertexPullerConfiguration *puller,
	VertexShaderInvocation baseVertexShaderInvocation, VertexShader vertexShader
);
 ...
 */
void gpu_runTriangleClipping(GPUTriangleList *output, const GPUTriangle *input);
 ...
 */
void gpu_runPerspectiveDivision(GPUPrimitive *primitive);
...
void gpu_runViewportTransformation(
	GPUPrimitive *primitive, size_t width, size_t height
);
 ...
 */
size_t gpu_roundDownPixelCoord(float coord);
 ...
 * @return closest integer
 */
size_t gpu_roundUpPixelCoord(float coord);
...
void gpu_restrictLineBorders(
	float *minX, float *maxX, float y, const Vec3 *edgeLine
);
...
void gpu_computeScreenSpaceBarycentrics(
	Vec3 *coords, const Vec2 *pixelCenter,
...
void gpu_createFragment(
	GPUFragmentShaderInput *fragment, const GPUPrimitive *primitive,
	const Vec3 *barycentrics, const Vec2 *pixelCoord
);
...
void gpu_perFragmentOperations(
	GPU gpu, const GPUFragmentShaderOutput *fragment, size_t x, size_t y
);
 ...
 * @param gpu GPU handle
 */
void gpu_initPrimitive(GPUPrimitive *primitive, GPU gpu);
...
void gpu_createSubPrimitive(
	GPUPrimitive *subPrimitive, const GPUPrimitive *primitive,
	const GPUTriangle *clippedTriangle
);
 ...
 * @param primitive input primitive
 */
void gpu_initTriangle(GPUTriangle *triangle, const GPUPrimitive *primitive);
...
void gpu_rasterizeTriangle(
	GPU gpu, const GPUPrimitive *primitive, size_t width, size_t height
);
 ...
 */
void cpu_drawTriangles(GPU gpu, size_t nofVertices);
 ...
 * Zkopirujte projekt na merlin a spusťte skript:
 * <b>./merlinCompilationTest.sh</b>.
