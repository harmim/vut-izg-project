...
TEST_CASE("gpu_computeGLVertexID should compute gl_VertexID.")
{
	...
	{
		REQUIRE(gpu_computeGLVertexID(nullptr, 0) == 0);
		REQUIRE(gpu_computeGLVertexID(nullptr, 1) == 1);
		REQUIRE(gpu_computeGLVertexID(nullptr, 2) == 2);
		REQUIRE(gpu_computeGLVertexID(nullptr, 3) == 3);
		REQUIRE(gpu_computeGLVertexID(nullptr, 4) == 4);
		REQUIRE(gpu_computeGLVertexID(nullptr, 5) == 5);
	}
}
...
TEST_CASE("gpu_runPrimitiveAssembly should construct primitive.")
{
	...
	{
		puller.heads[a].buffer = (void *) nullptr;
...
TEST_CASE(
	"phong_vertexShader should project vertex to clip-space and move vertex "
	"position and normal to output.")
{
	Mat4 viewMatrix;
...
TEST_CASE("phong_fragmentShader should compute phong color.")
{
	...
	REQUIRE(equalFloats(outputFragment.color.data[2], 3.1046023965e-01f));
...
TEST_CASE("Application should render correct image.")
{
	...
	{
		SDL_LogError(
			SDL_LOG_CATEGORY_APPLICATION, "SDL_Init fail: %s\n",
			SDL_GetError());
		exit(1);
	}
	...
	SDL_Window *window = SDL_CreateWindow(
		applicationName, SDL_WINDOWPOS_CENTERED,
	...
	{
		SDL_LogError(
			SDL_LOG_CATEGORY_APPLICATION, "SDL_CreateWindow fail: %s\n",
			SDL_GetError());
		exit(1);
	}
	...
	if (surface->w == groundTruth->w && surface->h == groundTruth->h)
	{
